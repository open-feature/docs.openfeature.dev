...
the `Metadata()` method is very simple to implement, it simply needs to return a [`Metadata` object](../specification/sections/providers#requirement-21), currently this object only requires one field - `Name`
...
The `Hooks()` method gives the `go-sdk` access to `Provider` hooks, this sits outside the scope of this tutorial, so for now we will just return an empty slice of hooks. [Link to spec.](../specification/sections/providers#requirement-210)
