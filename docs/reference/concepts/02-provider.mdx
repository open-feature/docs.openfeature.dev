...
If an error condition is encountered in your provider, an error should be [indicated](/docs/specification/types#error-code) by throwing or returning an error, as language idioms dictate.
...
You should publish your provider on the appropriate package management repository for your language, keeping in mind the [naming conventions](https://docs.openfeature.dev/docs/reference/concepts/provider#naming-recommendations).
